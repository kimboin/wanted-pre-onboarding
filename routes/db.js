...
router.get('/db/recruit/create', function(req, res) {
  var sql = 'CREATE TABLE RECRUIT ('
  sql += 'RECRUIT_ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY, '
  sql += 'COMPANY_ID INT(20) NOT NULL, '
  sql += 'POSITION VARCHAR(100) NOT NULL, '
  sql += 'COMPENSATION INT(15) NOT NULL, '
  sql += 'CONTENT CLOB NOT NULL, '
...
router.get('/db/recruit/drop', function(req, res) {
...
router.get('/db/recruit/insert', function(req, res) {
  maria.query('INSERT INTO RECRUIT(COMPANY_ID,POSITION,COMPENSATION,CONTENT,SKILLS)'
  +'VALUES("백엔드 주니어 개발자", 1000000, "원티드랩에서 백엔드 주니어 개발자를 채용합니다. 자격요건은..", "Python")', function(err, rows, fields) {
      if(!err) {
        res.send(rows); // response send rows
      } else {
        console.log('err : ' + err);
...
router.get('/db/recruit/select', function(req, res) {
...
router.get('/db/recruit/delete', function(req, res) {
...
router.get('/db/company/create', function(req, res) {
  var sql = 'CREATE TABLE COMPANY ('
  sql += 'COMPANY_ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY, '
...
router.get('/db/company/drop', function(req, res) {
...
router.get('/db/company/insert', function(req, res) {
...
router.get('/db/company/select', function(req, res) {
...
router.get('/db/company/delete', function(req, res) {
